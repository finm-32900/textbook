...
def pull_fred_repo_data(start_date, end_date, ffill=True):
    ...
    if ffill:
        forward_fill = ['OBFR', 'DPCREDIT', 'TREAST', 'TOTRESNS', 'WTREGEN', 
                        'WALCL', 'CURRCIR', 'RRPONTSYAWARD', 'WSDONTL']
...
if __name__ == "__main__":
    ...
    end_date = today
    df, df_focused = pull_fred_repo_data(start_date, end_date)
    filedir = Path(DATA_DIR) / 'pulled'
    filedir.mkdir(parents=True, exist_ok=True)
    df.to_parquet(filedir / 'fred_repo_related_data_all.parquet')
    df_focused.to_parquet(filedir / 'fred_repo_related_data.parquet')
