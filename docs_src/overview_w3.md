...
- **The case study**: walk through the Fama-French pipeline end to end---automated CRSP and Compustat pulls feeding factor construction, with unit tests at the end. This is the same automated-query pattern from Week 2, now embedded in a pipeline that rebuilds itself with one command.
